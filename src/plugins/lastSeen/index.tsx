...
function formatDate(ts: number): string {
    ...
    if (isToday) return "Today at {time}".replace("{time}", timeStr);
    if (isYesterday) return "Yesterday at {time}".replace("{time}", timeStr);
    
    const dateStr = date.toLocaleDateString(locale, { day: "numeric", month: "short" });
    return "{date} at {time}".replace("{date}", dateStr).replace("{time}", timeStr);
}
...
function LastSeenText({ userId }: { userId: string; }) {
    ...
    if (isOnline) {
        if (status === "idle") content = "Idle";
        else if (status === "dnd") content = "Do Not Disturb";
        else if (status === "streaming") content = "Streaming";
        else content = "Online";
    } else if (lastSeen) {
        content = formatDate(lastSeen);
    } else {
        content = "Not tracked yet";
    }
